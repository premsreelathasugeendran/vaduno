...
## Status: v0.7.0, new, and honest about it

Read this before you put it anywhere near real money.

- **Published recently. No known users. Never run in production.** The tests are thorough (1,201 passing across eight packages, including concurrency and adversarial cases, plus 3 capability-gated skips — two need native ML-DSA in node:crypto, one needs a live Postgres — and every skip says so rather than silently passing) but tests are not production. npm reports a few hundred weekly downloads; that is registry mirrors, security scanners and the author's own CI — the daily curve is a spike on each publish day collapsing to near zero between releases, which is what automated traffic looks like and what "nobody depends on this yet" looks like. The weekly total moves constantly and means nothing, so no figure is quoted here.
...
- **In-process, it can be routed around — so don't run it in-process with the key.** A library the agent's own process imports is a guardrail against a *confused* agent, not a *compromised runtime* — an injected agent holding a raw wallet key can simply not call it. Two shipped configurations close that hole by taking the key out of the agent's reach: **Stripe Issuing**, where the guard answers the card authorization itself and the network enforces the answer; and the **out-of-process signer host** (`createSignerHost` / `remoteSigner` in [`@vaduno/cloudflare`](packages/cloudflare)), where the raw key lives in a separate process or Durable Object whose *only* exposed capability is the policy-gated `signTypedData` — the agent process holds an address and a transport, and a full compromise of it can only *ask* for signatures, each one policed and ledgered on the key-holder's side (`npm run demo:keyless` spawns both processes and proves it; a real Base Sepolia settlement has been made through this arrangement). Honest residuals: compromise of the *host* process still owns the key, the transport authenticates nobody by itself, and a guard the deployer simply never wires in protects nothing — see the package README. Know which mode you are in: the signer host first ships in **0.7.0** — `@vaduno/cloudflare@0.6.1` on npm contains only the in-process wrapper, which is the routable-around configuration.
...
Writing your own limiter is expected, and there's an oracle for it: [`spend-limiter-conformance.ts`](packages/guard/test/spend-limiter-conformance.ts) runs 38 cases against **two independent handles on one backing store**. A deliberately naive check-then-act implementation passes all 33 sequential cases and fails exactly the 5 concurrent ones (re-measured against the current suite, 2026-08-09) — which is precisely how this class of bug reaches production.
...
- ✅ **Out-of-process signer host** (`createSignerHost` / `remoteSigner`, same package) — the raw key in a separate process or Durable Object whose only exposed capability is the policy-gated `signTypedData`; the agent process is keyless by construction. Proven by a two-OS-process demo (`npm run demo:keyless`) and a real Base Sepolia settlement signed across the boundary
