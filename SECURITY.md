...
| Payment settled on the wrong chain | `intent.network` is a first-class field and `policy.networks.allow` / `.block` constrain it (case-insensitive, no namespace wildcards; CAIP-2 ids compared **structurally**, the eip155 reference numerically); an intent that states no network under a policy that declares one is denied `NETWORK_MISSING`. **A policy with no `networks.allow` list is chain-blind** — a block list alone admits every chain it does not name, which is why the x402 gate refuses block-only deployments (`NETWORK_UNGATED`) — currency is not a chain, and USDC on two testnets produced the identical intent. See limitation 11 |
| Blocklisted chain respelled by a hostile seller (`eip155:084532` for `eip155:84532`, hex chain id, whitespace variant — or the OTHER wire spelling: the x402 v1 name `"base-sepolia"` for the CAIP-2 id `"eip155:84532"`) | The network id arrives in a 402 from an untrusted counterparty, so CAIP-2-shaped ids are parsed structurally and canonicalized (leading zeros fold numerically on `eip155`); an id that does not parse is denied `NETWORK_UNPARSEABLE`, never passed through, and a policy entry that cannot itself be canonicalized poisons the whole constraint (`NETWORK_POLICY_INVALID`). For the EVM chains the x402 registry defines, the v1 name and the CAIP-2 id canonicalize to ONE comparison key (curated alias table), so a seller cannot answer in whichever spelling the blocklist did not name; non-EVM v1 names are NOT aliased — constrain both spellings yourself. **This row was FALSE before 0.6.1 twice over:** comparison was trim+lowercase text (non-canonical spellings passed), and the two wire spellings of one chain were disjoint keys (the reattack paid on the v1 spelling of a CAIP-2-blocked chain) |
